...
function infinityScroll() {
  ...
  if (maxScroll === window.scrollY && location.hash === "#more-trends") {
    ...
    page++;
    if (page <= maxPages) trendingMovieViewMore(); //Se manda a llamar la función de consulta
  }
  if (maxScroll === window.scrollY && location.hash === "#more-popular") {
    //Se aumenta la página
    page++;
    if (page <= maxPages) popularMovieViewMore(); //Se manda a llamar la función de consulta
  }
  ...
  if (maxScroll === window.scrollY && location.hash.startsWith("#search")) {
    ...
    //Se aumenta la página
    page++;
    if (page <= maxPages) getMovieBySearch({ query }); //Se manda a llamar la función de consulta
  }
...
function movieAddToFavorite(movie) {
  ...
  if (localStorage.getItem("movie liked") === null) {
    ...
    const moviesInlocalStorage = JSON.parse(
...
const api = axios.create({
  baseURL: "https://api.themoviedb.org/3/a7b5c4c3473cc3d4505705c7fc5bb4ab",
  headers: {
...
async function trendingMovieViewMore() {
  try {
    ...
    if (page <= 1) trendingCardsContainerList.innerHTML = "";
    /* se llama a la función que genera las movie cards */
    const moviesList = res.data.results;
...
async function popularMovieViewMore() {
  try {
    ...
    maxPages = res.data.total_pages; //se establce el número máximo de páginas que habrá en esta vista
    if (page <= 1) popularCardsContainerList.innerHTML = "";
    /* se llama a la función que genera las movie cards */
    const moviesList = res.data.results;
...
function favoriteMovieView(movieList) {
  ...
  if (movieList === null || movieList) {
    ...
    lang === "en-US"
      ...
      : lang === "es"
      ? (p.innerText = "Agrega una película que te guste")
      : lang === "fr"
      ? (p.innerText = "Ajouter un film aux favoris")
...
async function categoryMoviePreview() {
  ...
  try {
    ...
    const dropItemsContainer = document.getElementById(
      "dropdown-items-container"
    );
    const dropdownMobileItems = document.getElementById(
...
async function createMovieRelatedPoster(url) {
  ...
  const cardsContainer = document.querySelector(
    ".related-movie__cards-container"
  );
